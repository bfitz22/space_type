...
class Game {
    ...
    drawUFOs() {
        ...
        this.ufos.forEach((ufo, i) => {
            let dx = (ufo.x - 575) / (ufo.y - this.shieldHeight - 140);
            this.ctx.drawImage(ufoImg, 0, this.lights, 32, 32, ufo.x, ufo.y, 42, 42),
            ufo.x += dx,
            ufo.y += dy,
            ufo.drawText();
            if ((ufo.y > this.shieldHeight ) && 
                ((ufo.x > this.shieldCenter - (this.shieldRadius + 10)) && 
                (ufo.x < this.shieldCenter + (this.shieldRadius - 10))) ||
                (ufo.y > this.shieldHeight + 30) && 
                ((ufo.x > this.shieldCenter - (this.shieldRadius + 20)) && 
                (ufo.x < this.shieldCenter + (this.shieldRadius - 20))) || 
                (ufo.y > this.shieldHeight + 50) && 
                ((ufo.x > this.shieldCenter - (this.shieldRadius + 40)) && 
                (ufo.x < this.shieldCenter + (this.shieldRadius - 40))))
                {
    ...
    drawSaucers() {
        ...
        this.saucers.forEach((saucer, i) => {
            let dx = (saucer.x - 575) / (saucer.y - this.shieldHeight);
            this.ctx.drawImage(saucerImg, 0, this.lights, 32, 32, saucer.x, saucer.y, 42, 42),
            saucer.x += dx,
            saucer.y += dy,
            saucer.drawText();
    
            if (saucer.y > this.shieldHeight ) {
                if (this.sound.isPlaying) {
                    this.sound.powerDown.play();
                }
    ...
    drawWings() {
        ...
        this.wings.forEach((wing, i) => {
            let dx = (wing.x - 575) / (wing.y - this.shieldHeight - 25);
            this.ctx.drawImage(wingImg, 0, this.lights, 32, 32, wing.x, wing.y, 42, 42),
            wing.x += dx,
            wing.y += dy,
            wing.drawText();
    
            if (wing.y > this.shieldHeight ) {
                if (this.sound.isPlaying) {
                    this.sound.powerDown.play();
                }
    ...
    drawBonus() {
        this.bonuses.forEach((bonus, i) => {
            this.sound.isPlaying ? bonus.bonusSound.play() : bonus.bonusSound.pause();
